...
export async function runBenchmark(iterations: number, epsilon: number) {
  ...
  const response = await fetch("./polyline_10000000.json");
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const count = dp_full_inplace_noalloc(
      ptrIn,
      points * 2,
      epsilon,
      ptrKeep,
  ...
  console.log(
    `Среднее: ${(times.reduce((a, b) => a + b, 0) / times.length).toFixed(3)} ms`,
  );
